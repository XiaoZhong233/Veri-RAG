...
| Tool | 作用 |
|---|---|
| `search_room_offers` | 按城市、公寓、学校/地标、日期、租期、预算、房型和库存查询推荐，返回参考价格 |
| `get_residence_details` | 查询公寓地址、设施、车站、学校和地标 |
| `check_room_offer_availability` | 根据房源 ID 和日期/周数核验库存，计算参考周价和参考总价 |
| `list_residences` | 查询公寓名单和基础地址信息 |
| `get_inventory_summary` | 汇总城市维度的公寓、房型和库存情况 |

每次模型只会获得当前意图对应的 Tool，减少模型探索式地反复调用多个接口。推荐结果最多返回 4 个公寓、每个公寓最多 2 个选项、最终最多 6 个房型；不足时不会使用距离过远或不满足硬条件的公寓凑数。面向客户的参考价格可在回答中展示，最终报价和可订状态仍需 Londonist 顾问确认；采购价、底价和代理结算价等内部信息不对外披露。
...
| `LLM_TIMEOUT` | `3m` | 模型单次 HTTP 请求总超时，超时后取消底层请求 |
...
| `RAG_CHAT_RESPONSE_TIMEOUT` | `3m` | 用户问答模型生成总超时，包含同步 Tool 调用循环 |
