...
微信渠道启用内部 `allowHumanHandoff` 标记，和房源意图共用初次分类请求，读取现有最近对话。
若带历史的结果为转人工，再仅用当前消息做一次无历史确认；未确认则禁止转接，
按原历史重判普通咨询意图。普通问题仍只调用一次；确认转接通常两次，拒绝旧意图后重判最多三次。
模型输出 `HUMAN_HANDOFF` 后直接执行接待人员分配，不再生成普通 RAG 回答。
不再使用原来的关键词包含匹配；网页提交的 JSON 不能启用内部动作。

分类提示覆盖明确转接、否定、询问人工服务时间、旧转接意图隔离和合并消息中的撤回。
当前消息必须独立表达转人工要求；“好的”等依赖旧对话的确认不再触发转接。
