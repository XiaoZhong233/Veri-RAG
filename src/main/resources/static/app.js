...
function formatDate(value) { return value ? new Intl.DateTimeFormat(window.i18n?.formatLocale?.() || 'zh-CN', {dateStyle: 'medium', timeStyle: 'short'}).format(new Date(value)) : '-'; }
...
async function loadResidences() {
    ...
    try {
        const query = new URLSearchParams({name: $('#residence-query').value.trim(), city: $('#residence-city').value, region: $('#residence-region').value, page: state.residencePage, size: state.residenceSize});
...
function renderResidences(records) {
    ...
    body.innerHTML = records.map(residence => {
        ...
        const adminActions = isAdmin() ? `<details class="row-menu"><summary title="更多操作" aria-label="更多操作"><i class="ri-more-2-fill" aria-hidden="true"></i></summary><div class="row-menu-popover"><button class="text-button" data-residence-action="edit" data-id="${residence.id}" type="button">编辑地址</button><button class="text-button danger" data-residence-action="delete" data-id="${residence.id}" data-name="${escapeHtml(residence.name)}" type="button">删除</button></div></details>` : '';
        const actions = `<div class="row-actions"><button class="text-button" data-residence-action="detail" data-id="${residence.id}" type="button">查看详情</button>${adminActions}</div>`;
        return `<tr><td><strong class="residence-name">${escapeHtml(residence.name)}</strong><small>${escapeHtml(residence.sourceId)}</small></td><td>${escapeHtml(residence.city || '-')}</td><td><span class="region-tag ${escapeHtml(residence.region || '')}">${escapeHtml(residenceRegionNames[residence.region] || residence.region || '-')}</span><small>${escapeHtml(residence.zone || '-')}</small></td><td><span class="residence-address">${escapeHtml(residence.address || '地址待补充')}</span>${coordinates ? `<small>${escapeHtml(coordinates)}</small>` : ''}</td><td>${escapeHtml(residence.station || '-')}</td><td>${mapUrl ? `<a class="map-link" href="${escapeHtml(mapUrl)}" target="_blank" rel="noopener noreferrer">查看地图 <i class="ri-external-link-line" aria-hidden="true"></i></a>` : '-'}</td><td>${actions}</td></tr>`;
    }).join('');
